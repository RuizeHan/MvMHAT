...
if __name__ == '__main__':
    stan=Munkrs(element_dim=C.element_dim, hidden_dim=C.hidden_dim, target_size=C.target_size, biDirenction=C.bidrectional, minibatch=C.batch_size, is_cuda=C.is_cuda, is_train=True).cuda()
    x=torch.randn(1, 110, 110).cuda()
    stan(x)