...
def read_loader(dataset_name):
    dataset = Loader(frames=1, views=4, mode='test', dataset=dataset_name,detection_mode='origin')
...
if __name__ == "__main__":
    ...
    if C.INF_ID is None:
        ...
        for epoch_i in range(C.start_epoch,C.end_epoch+1):
            ...
            if len(name_list)>0:
                ...
                for dataset_name in C.TEST_DATASET:
                    run(output_file="./output/" + output_dir + "_" + C.inference_mode + '/'+model_name.split('.')[0]+'.'+model_name.split('.')[1] + "_" + C.inference_mode + "/", display=C.DISPLAY, dataset=dataset_name,model=[resnet_dhn_model.feature_extractor, resnet_dhn_model.dhn_model])
            else:
                continue
    else:
        # TODO 测试单个模型
        ...
        for dataset_name in C.TEST_DATASET:
            run(output_file='./output/'+C.INF_ID + "_" + C.inference_mode + '/', display=C.DISPLAY, dataset=dataset_name, model=[resnet_dhn_model.feature_extractor,resnet_dhn_model.dhn_model])
